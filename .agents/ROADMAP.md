...
3. **[Major Update] System Stability Overhaul — Exhaustive Diagnosis & Refactoring** *(ACTIVE)*
   ...
   - **Active plan (awaiting approval)**: XC-1 broad-except narrowing, slice 1
     (data-pipeline modules) → `.agents/plans/02_error_handling_narrowing.md`
     (target v0.27.5, Patch). Shipped so far: G17/G18/G19 (v0.27.3), G17 S3 (v0.27.4).
     Remaining S2: god-file decomposition CM-1/PL-1/DB-2; XC-1 slices 2+ (god-file
     excepts, `model_setup.py`); XC-4 plugin timers/logging.
