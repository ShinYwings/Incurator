...
## Audit: the external-file surface cannot tell "missing" from "not allowed", anywhere

**Swept the whole external-file path after the Zotero/iCloud failure.** The
user's suspicion was right — this is not one bad line, it is a missing concept
repeated across both halves of the codebase.

### F1. Nothing distinguishes a permission denial. Anywhere.

`PermissionError` appears **once** in the entire backend
(`commands/core.py:121`), and there it is redundant — `PermissionError` is a
subclass of `OSError`, which the same clause already catches. On the plugin
side, `accessSync` appears **once** (`plugin/main.ts:1400`), against 41
`existsSync`/`statSync` calls.

So no code on the ingest, resolve, or render path can produce the sentence "you
do not have access to this folder."

### F2. `exists()` is used as if it meant "readable". It does not.

Measured readability checks (`os.access` / `R_OK`) across the external-file
modules:
...
Under macOS TCC, `stat` succeeds and `open` fails. Every one of these checks
returns True for a file the process cannot read a byte of. The name of the
deciding helper says it out loud:
...
"Existing" is the wrong predicate. It picks a file, declares `ok: True`, and
hands the path downstream to a parser that then fails with
`Cannot parse PDF ...` (`parsers/pdf.py:156`) — which reads as a corrupt file.

### F3. The error message actively misdirects, in two places

- `parsers/pdf.py:156` wraps *any* exception as `Cannot parse PDF {name}: {e}`.
  A permission denial arrives dressed as a parse failure, sending the user to
  check a file that is perfectly fine in Finder.
- `zotero_tools.py` reports **`"Zotero attachment file not found"`** when it
  cannot produce a path. If the file exists and is merely unreadable, that
  sentence is false, and it is the sentence the user acts on.

### F4. The two Zotero directories are fused into one list

`zotero_root_candidates()` returns a single list mixing two different things:

- **data directories** — `~/Zotero`, config roots. Hold `zotero.sqlite`; the
  *index* that resolves `zotero:<key>` and records where attachments live.
- **attachment directories** — from `extensions.zotero.baseAttachmentPath` and
  `extensions.zotmoov.dst_dir`, appended by
  `discover_zotero_base_attachment_path`. Hold the actual PDF bytes; commonly
  iCloud, Dropbox, or an external volume.

The consequence is visible at `zotero_tools.py:30-35`: `_db_candidates` takes
that mixed list and probes `<root>/zotero.sqlite` for **every** entry, including
attachment directories, which by definition never contain the database. Harmless
in effect, but it is the fusion made concrete — and the naming (`roots`) gives a
reader no way to tell which kind they are holding.

**Why the distinction is load-bearing rather than pedantic**: they are separate
macOS grants. The data directory being readable says nothing about the
attachment directory, and that exact split is the observed failure — discovery
succeeds, so the precise path is known, and `open()` is denied.

### F5. The plugin has the same shape

`ExternalPdfView.ts` gates on `existsSync` at `:169`, `:294`, `:861`, and at
`:1385` goes straight from `existsSync(doc.path)` to
`readFileSync(doc.path)` — the one place where the gap between the two is a
guaranteed throw rather than a possibility.

### Direction for a plan

1. **One helper that answers the real question.** Replace `_first_existing_pdf`
   and its siblings with a check that attempts access, and returns a
   three-valued answer — found / missing / **denied** — rather than a bool.
2. **Carry `denied` to the surface.** `resolve_pdf` must stop reporting "not
   found" for a file it can see; the parser boundary must not relabel a
   permission error as a parse error.
3. **Split the two Zotero directory kinds** in the type, not just in a comment,
   so `_db_candidates` stops probing attachment directories and a reader cannot
   confuse them. This is the fix for a confusion that has recurred.
4. **Say it in the UI.** The dashboard knows which sources are unreadable; that
   is a better place to surface "grant access to this folder" than a job error.
