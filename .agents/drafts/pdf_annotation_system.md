...
## Completed / Split-Out PDF-Domain Scope

- PDF / Zotero asset-location management, external-image asset routing, reload
  relativepath handling, and add-source button state were split out and shipped
  through the PDF handling work up to v0.12.0. They are no longer active scope
  for this annotation-system draft.

## Remaining PDF-Domain Scope
...
## Files Likely Involved (remaining scope)
- `plugin/src/pdf/*` (annotation UI and in-PDF text search)
- PDF parser / annotation reader in `backend/src/curator/`
