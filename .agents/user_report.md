...
- **10. [마이너 업데이트] 기존 Diff Viewer UI/UX 개선**
  - 현황: `plugin/src/ui/diffViewer.ts` (530줄)에 Accept/Reject/Accept All/Reject All 버튼 및 키보드 단축키(Y/N/Enter/Escape), 헝크 네비게이션(↑/↓, Shift+Tab/Tab), CodeMirror 인라인 데코레이션이 이미 구현되어 있음.
  - 요구사항 (UI/UX 폴리시): 이미 동작하는 DiffViewer를 기반으로 세련도와 사용성을 높이는 개선. 예: 헝크 간 이동 UX 개선, 변경된 라인 수 요약 배지, 키보드 단축키 힌트 표시, 다크/라이트 테마 대응 색상 등.
