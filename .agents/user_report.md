...
- **8. 스펙 문서명 정적화(Static) 및 Git-Native 버전 관리 전환 작업**
  - 상태: 구현 계획 승인 완료 (`.agents/plans/01_static_specs_refactoring.md`). 과거 `archives/` 정리 및 플랜 네이밍 규칙 일원화 등 사전 클린업 작업이 방금 완료되었습니다. 이제 피처 브랜치에서 백엔드/플러그인 전역의 `_v0.3.2` 꼬리표 의존성을 떼어내는 실제 코딩 작업에 착수합니다.
...
- **4. PDF 및 정제된 지식(Atom, Concept) 내 수학 수식 누락 문제 해결**
...
- **5. 지식 정제용 LLM과 쿼리 확장(HyDE)용 LLM 설정 분리 및 UI/CLI 노출**
...
- **6. GraphRAG급 엔티티 통합(Entity Resolution), 노이즈 필터링 및 보관소 용량 관리(Vault Quota) 아키텍처 설계**
...
- **7. 전역적 사고(Global Sensemaking)를 위한 계층적 군집화 알고리즘 설계 플랜 작성**
...
- **10. 옵시디언 에이전트 UI: IDE 스타일의 Diff Viewer 도입**
