...
- **10. [마이너 업데이트] 기존 Diff Viewer 동작 검증 및 UI/UX 개선**
  - 현황: `plugin/src/ui/diffViewer.ts` (530줄)에 Accept/Reject/Accept All/Reject All 버튼, 키보드 단축키(Y/N/Enter/Escape), 헝크 네비게이션, CodeMirror 인라인 데코레이션이 구현되어 있음.
  - 요구사항: 코드가 존재한다고 기대대로 동작한다는 보장이 없음. 실제 에이전트가 파일을 편집할 때 DiffViewer가 올바르게 트리거되는지, Accept/Reject가 실제 파일에 반영되는지, 엣지 케이스(멀티 헝크, 대용량 diff 등)에서 버그가 없는지 검증 선행. 검증 결과에 따라 버그 수정 및 UX 개선(단축키 힌트, 테마 대응, 헝크 이동 UX 등) 진행.
