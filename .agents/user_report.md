...
> **[기능 제안] Zotero급 로컬 DB 클라우드화 (Syncthing 자동 동기화)** — 구현 완료, PR #6 (`release/v0.4.0`)에 포함. 머지 시 본 항목과 플랜(`syncthing_auto_sync*.md`) 삭제.
> - One-writer-per-file 토폴로지, 구조적 루프 방지(해시 가드 없음), LWW+tombstone 병합, 충돌 파일 자동 병합/보관, reference-mode 경로 보존, 플러그인 자동 동기화(on-load/watcher/poll/ribbon) + 설정 4종.
> - 엣지 케이스 4종(충돌 파일 / 다운로드 레이스 / 덮어쓰기 유실 / 대용량 프리징) 모두 처리. 백엔드 485 + 플러그인 282 테스트 통과.
>
