...
8. **Local CI Validation**: Before finalizing, you MUST run all local checks: `uv run pytest ./backend/`, `ruff check ./backend/`, `mypy ./backend/`, and the plugin's `npx vitest run -c ./plugin/vitest.config.ts`. Ensure the entire system is intact.
