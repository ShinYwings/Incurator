...
1. **Batch & Version Planning**: Read `.agents/user_report.md` (ignoring Blocked items) and group related items into a single Batch Release. Cross-reference each candidate item against `.agents/plans/` roadmap skeletons to understand which milestone it belongs to. **CRITICAL — Implementation Status Check**: Before finalizing the batch, quickly verify each candidate item's actual status in the codebase (grep for key symbols, check relevant files). If an item is already implemented, flag it for the user to remove from user_report rather than silently including it. Decide whether this batch warrants a Patch, Minor, or Major version bump. **CRITICAL**: If the update is Minor/Major and includes breaking schema changes, you MUST plan and write a data migration script.
