...
10. **Version Bump & Changelog**: Update the version strings in all relevant configuration files (`pyproject.toml`, `package.json`, `manifest.json`) AND update `CHANGELOG.md` with the release notes for this version. **MINOR/MAJOR SPEC-LINE SYNC (mandatory whenever the `MAJOR.MINOR` line changes — e.g. `0.16.x → 0.17.0`)**: the backend `__version__` is read from installed package metadata, and `backend/tests/test_spec_sync.py` hard-asserts that (a) its `ACTIVE_VERSION` constant matches the backend minor line and (b) every static spec title (`docs/specs/*/{SCHEMA,SYSTEM_BEHAVIOR,PLUGIN_SCHEMA,SEARCH_ENGINE_SCHEMA}.md` first line) declares that version. So on any minor/major bump you MUST also (1) set `ACTIVE_VERSION` in `test_spec_sync.py` to the new `X.Y.0`, and (2) bump the `(vX.Y.Z)` suffix in all four spec-file titles. This passes locally only after the dev venv is reinstalled (`uv pip install -e backend[dev,mcp]`) so the metadata refreshes; CI installs fresh and will fail the bump otherwise. Pure patch bumps that keep the same minor line do not need this.
