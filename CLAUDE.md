...
**VERSION BUMP IS MANDATORY FOR ALL HOTFIXES**: Even when the user says "just push this as hotfix", Step 10 (Version Bump & Changelog) is NON-NEGOTIABLE. A hotfix branch named `hotfix/vX.Y.Z` MUST have matching version strings in `pyproject.toml`, `package.json`, and `manifest.json` before the PR is opened. The CI `version-consistency` job will block the merge if they disagree — this is the hard gate. The version bump is what triggers the Obsidian update toast for installed users; skipping it silently breaks the self-update mechanism.
...
| `search.py` | DB-native search: SQLite FTS5/BM25 + chunk vector + RRF fusion + LLM reranking (qmd retired in v0.3.2) |
