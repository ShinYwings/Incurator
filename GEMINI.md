...
3. **The Document Sandbox**: You fully manage `.agents/` tracking files, `GEMINI.md`, `CHANGELOG.md`, and the `docs/` folder. You MUST immediately `git add` and `git commit` any rule or workflow updates you make (e.g., modifying `GEMINI.md` or `.agents/workflows/`). However, do NOT auto-commit updates to `docs/` or `CHANGELOG.md`; those changes are tied to the milestone's substantive implementation and should be committed alongside the relevant code.
