...
2. **Master Sync & Branch Creation**: Run `git checkout master` and `git pull`. Then immediately create the new feature/release branch for the upcoming milestone using `git checkout -b <new_branch_name>`. (Never work directly on the master branch).
3. **State Initialization**: Update `.agents/ROADMAP.md` to mark the completed plan as "shipped" and declare the next item as active. Truncate and reset `.agents/RELAY.md` with the new target plan and branch. **CRITICAL EXCEPTION for Intermediate Milestones**: If you are merely transitioning between sub-waves within an *active, unmerged PR* (e.g., P2 to P3), you skip this entire Post-Merge sequence. You MUST NOT truncate `RELAY.md`; instead, preserve context and only append the Next Action.
