...
3. **The Document Sandbox**: You manage the PM state tracking files (`.agents/RELAY.md`, `.agents/ROADMAP.md`, `.agents/USER_REPORT.md`, `.agents/drafts/`), your own rules (`GEMINI.md`), and `.agents/workflows/`. You are STRICTLY FORBIDDEN from modifying `.agents/plans/`, the `docs/` folder, or `CHANGELOG.md`; those are exclusively managed by the Executors. Furthermore, for Git commits, you MUST ONLY auto-commit `GEMINI.md` and files in `.agents/workflows/`. You MUST NEVER auto-commit changes to state tracking files (with the sole exception of initializing a new branch post-merge, as defined in §3.B).
...
When a final PR is merged, you MUST execute the following exact sequence to transition the State Machine and initialize the next milestone:
1. **Explicit Human Approval Mandate (Final PR Merges Only)**: The authority to approve and merge a final Pull Request belongs exclusively to the Human User. You MUST wait for the user to explicitly state "Merged" before initiating this post-merge transition. (Intermediate milestones within an active branch transition automatically without asking).
2. **Master Sync & IDLE Cleanup (CRITICAL)**: Run `git checkout master` and `git pull`. You MUST immediately sanitize the `master` branch by updating `.agents/ROADMAP.md` (mark the merged plan as "shipped" and set Active Milestone to "None (System IDLE)") and truncating `.agents/RELAY.md` to a minimal IDLE stub. You MUST commit this cleanup directly to the `master` branch (`git commit -am "chore(agents): reset master state to IDLE"`) and push it. This guarantees that `master` never permanently holds an ACTIVE state from a merged PR.
3. **Branch Creation & State Initialization**: Create the new feature/release branch for the upcoming milestone from `master` using `git checkout -b <new_branch_name>`. Update `.agents/ROADMAP.md` to declare the next item as active, and reset `.agents/RELAY.md` with the new target plan and branch. **CRITICAL EXCEPTION for Intermediate Milestones**: If you are merely transitioning between sub-waves within an *active, unmerged PR* (e.g., P2 to P3), you skip this entire Post-Merge sequence. You MUST NOT truncate `RELAY.md`; instead, preserve context and only append the Next Action.
4. **Commit the Initialization (Exception)**: As an explicit exception to the Document Sandbox rules, you MUST `git add` and `git commit` the updated `ROADMAP.md` and `RELAY.md` on the newly created branch (e.g., `chore(project): initialize milestone X`). This prepares the branch so the Executors can start coding immediately.
5. **Cleanup**: Safely delete the old merged local branch using `git branch -D <old_branch>`.
6. **Executor Handoff**: Set clear instructions in `RELAY.md` for the Executors.
7. **Final Output Constraint**: Do not output generic phrases like "Execute the workflow." Instead, your final output MUST be the actual review content, your architectural verdict, and an explanation of the state transition. Write it plainly so the user understands the context. Do not provide copy-paste prompts for the Executors; they will read the updated `.agents/RELAY.md` automatically based on your review.
...
- **Review Criteria (Rigorous Validator & Holistic Architect):** Do not merely pattern-match against literal rules. You must evaluate the system both microscopically and holistically:
  ...
  7. **Local CI Validation (Environment Integrity):** You must enforce that the production runtime environment (`.venv`) is strictly separated from the development/testing environment (`.venv-dev`). When validating code or running tests (e.g., `pytest`, `ruff`, `mypy`), you MUST use the `scripts/backend-check` wrapper (e.g., `scripts/backend-check pytest`) instead of invoking the tools directly. This ensures that dev-only tools do not pollute the production deployment and prevents the creation of localized tool caches inside the `backend/` directory.
