...
지식은 고정된 데이터가 아니라 **"컴파일"**되고 **"진화"**하는 생태계입니다. InCurator는 지식 저장소를 딥러닝 학습 과정처럼 관리합니다.
- **순방향 빌드 (Forward Pass)**: `wiki add`로 소스를 등록하고 L1~L3 계층을 형성하며, `wiki curate`를 통해 최종 L4 Exhibition으로 지식을 합성합니다.
- **피드백과 손실 신호 (Loss Signal)**: 대화 중 발견된 오류나 인간의 수정 사항은 시스템의 '오차'를 나타내는 신호가 됩니다.
- **역방향 동기화 (Backward Pass / Sync)**: `wiki sync`는 이 신호를 바탕으로 그래프를 역추적(Backpropagation)하여 논리적 모순을 해결하고, 연관된 상위 개념들을 재작성하여 시스템 전체의 무결성을 실시간으로 회복(Self-Healing)합니다.
