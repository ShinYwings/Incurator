...
단순 요약과 원자화 등 반복적인 데이터 컴파일(`Summary → Atoms → Concepts`)은 가벼운 로컬 SLM(Ollama)에 맡기고, 고성능 상용 추론 모델(Claude, OpenAI, Gemini 등)은 대화와 심층 합성에만 집중시켜 토큰 비용을 최대 90% 이상 절감합니다.
