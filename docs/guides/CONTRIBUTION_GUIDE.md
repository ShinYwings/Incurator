...
- **Linting & Types**: Python code in `backend/` must pass `ruff` and `mypy`. The authoritative Ruff rule selection is declared explicitly in `backend/pyproject.toml`; do not rely on a Ruff release's changing defaults. Plugin code in `plugin/` must pass TypeScript linting via `npm run lint`.
- **Backend Environments**: The repository-root `.venv` is the service/runtime environment updated by `./setup.sh`; it installs the backend runtime package and plugin build without dev check tools. The repository-root `.venv-dev` is the development and validation environment; install `backend[dev,mcp]` there for `pytest`, `ruff`, `mypy`, and MCP contract checks. Run backend checks with `scripts/backend-check pytest`, `scripts/backend-check ruff`, and `scripts/backend-check mypy`; the helper calls `.venv-dev/bin` directly and stores tool caches under repository `.cache/`. No virtualenv, `uv.lock`, or tool cache may be created under `backend/`.
- **Typed Dependencies**: Backend `mypy` runs with `backend/stubs/` on its search path. Prefer published type packages such as `types-PyYAML` when available; for untyped runtime libraries, add narrow local stubs that model only the APIs the code uses instead of suppressing missing imports globally.
