...
- **린팅 및 타입 체크**: 파이썬 코드는 `backend/`에서 `ruff`와 `mypy`를 통과해야 합니다. 기준 Ruff 규칙 선택은 `backend/pyproject.toml`에 명시되며, Ruff 릴리스에 따라 달라지는 기본값에 의존하면 안 됩니다. 플러그인 코드는 `plugin/`에서 타입스크립트 린트를 통과해야 합니다.
