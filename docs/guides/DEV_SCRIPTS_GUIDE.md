...
`plugin/deploy.sh` follows the same contract. If `OBSIDIAN_PLUGIN_DIR` is unset it builds in the local `plugin/` directory; if it is set it deploys to that path instead. This keeps the script usable on macOS and Linux:
...
Plugin validation can be run from the repository root:
