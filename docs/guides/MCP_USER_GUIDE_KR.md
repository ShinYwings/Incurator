...
- **저장된 지식이 필요 없는 메시지 (v0.81.0)**: 지식 질문이 아니라고 분류되면
  — "이 문단 번역해줘: …", "아래 글 요약해줘" — 검색을 건너뛰고 결과에
  `no retrieval: not a knowledge question` 경고와 빈 evidence pack이 담깁니다.
  번역 요청으로 볼트를 뒤지면 아무도 하지 않은 질문의 근거를 고르게 되고,
  검색어가 비면 메시지 자체로 폴백하므로 붙여넣은 본문 전체를 검색하게 됩니다.
  **분류는 이미 영어가 아닌 질문에만 실행됩니다**(비용 문제). 따라서 영어로 쓴
  "translate this: …"를 이 도구에 보내면 여전히 평소처럼 검색됩니다.
...
- **실패 의미**: `curate.yml`이 없거나 유효하지 않으면 `ok=false`를 반환하고
  `curation_plans` 행을 기록하지 않습니다. 검증은 항상 영속화보다 먼저
  수행됩니다.
