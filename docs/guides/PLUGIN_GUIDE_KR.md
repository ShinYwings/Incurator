...
- **텍스트 레이어에 없을 때 (v0.54.0)**: 일부 PDF는 본문에 표시되는 수식과 그림을
  ...
  *텍스트*를 검색해도 찾을 수 없습니다. 이제 어시스턴트는 페이지를 읽는 대신 볼 수
  ...
  그래도 해석되지 않는 pointer는 `<unresolved_cross_references>` 블록에 나열됩니다.
  그 블록은 사용자가 아니라 어시스턴트에게 하는 말입니다. 주변 자료가 그 대상에 대해
  establish하는 내용으로 설명하라고 모델에게 지시하며, 사용자가 보는 답변은
  어시스턴트가 무엇을 받았는지 받지 않았는지에 대한 보고가 아니라 논문에 대한
  것이어야 합니다.
