...
- **인용이 가리키는 논문으로 해석됩니다 (v0.56.0)**: `[8]`이 포함된 구절을 선택하고
  ...
  기본 PDF 뷰와 외부 PDF 뷰 모두에서 참고문헌 질문은 현재 다른 페이지를 읽고
  있어도 답변 전에 논문의 References 페이지를 가져옵니다. 목차에 위치가 있으면
  먼저 확인한 뒤, 필요할 때 문서 끝의 제한된 범위를 검색합니다. 저자·연도 형식의
  목록도 실제 페이지 발췌문으로 전달됩니다. “reference 90”처럼 번호를 지정하면
  일반 목록 제한보다 해당 항목을 먼저 가져오며, 같은 문서의 후속 질문에도
  참고문헌 근거가 유지됩니다. 일시적으로 실패한 읽기는 다시 시도합니다.
  출처 페이지 표시는 가져온 발췌문, 읽기 실패, 프롬프트 길이 때문에 잘린 텍스트를
  구분합니다. 이 검색이 모든 페이지를 확인했다고 주장하지는 않습니다.

  기본 뷰어는 질문마다 이 제한된 참고문헌 검색을 다시 수행합니다. 파일 경로만으로는
  같은 위치의 PDF가 수정되지 않았다고 확인할 수 없기 때문입니다. 외부 뷰어는
  문서 등록 식별자와 다시 불러올 때의 캐시 무효화를 사용합니다. 두 뷰어 모두
  백엔드 페이지 캐시는 계속 사용할 수 있습니다.
  ...
  **참고문헌과 매칭되지 않는 대괄호는 조용히 무시합니다.** `[8]`은 각주(`[^8]`),
  ...
  아무 말도 하지 않습니다. 인용이 없는 선택은 질문 자체가 참고문헌을 요구하는
  경우에만 참고문헌 절을 읽습니다.
