...
macOS는 파일 접근 권한을 Obsidian이 아니라 **백엔드 프로세스**에 부여합니다.
따라서 Finder에서 잘 보이고 더블클릭하면 잘 열리는 폴더인데도 여기서는 읽지
못할 수 있습니다. 이 탭은 바로 그 상황을 설명하기 위해 존재합니다. 예전에는
이런 경우가 아무 이유 없이 "PDF 파싱 실패"로만 나타났습니다.
...
`DENIED` 행에는 **Grant access…** 버튼이 붙습니다. 이 버튼은 권한을 줘야 하는
폴더를 여는데, 그 폴더가 행에 표시된 폴더와 같지 않을 수 있습니다. PDF가
...
macOS가 권한을 Incurator 백엔드가 아니라 Obsidian에 부여했을 수 있습니다. 즉
