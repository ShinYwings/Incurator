...
- **제공자 히스토리 제한**: 세션 전체는 세션 목록에서 계속 볼 수 있지만,
  각 제공자 요청에는 최근 대화 메시지 최대 4개(문자 48,000자 제한)만
  전달됩니다. 오래된 턴은 짧은 continuity summary로 남기므로 긴 세션에서
  이전 답변 전체를 매번 재전송해 Knowledge Off 질문이 느려지지 않습니다.
...
**popover가 답하기 전에 직접 가서 가져오는 것들.** 문서가 가리키는 대상에 대한
질문은 그 대상의 내용으로 답합니다. 이름만 보고 답하지 않습니다. CLI provider는
답변 도중에 무언가를 가져올 수 없으므로, 포인터는 턴 시작 전에 해석됩니다:

- **인용.** 문단 안의 `[12]`, 또는 아무것도 선택하지 않고 타이핑한
  "reference 12의 제목이 뭐야?" 모두 그 문서의 참고문헌과 대조됩니다. 번호를
  말하지 않고 참고문헌 목록을 물으면 목록 자체를 가져옵니다. 제목 인식은 영어·
  한국어·일본어·중국어를 지원하며, 책에서는 참고문헌 뒤에 붙는 색인을 넘어서까지
  탐색합니다.
- **상호참조.** "see Fig. 4", "3.2절", "Eq. (7)", "p. 214" — 드래그해서
  선택했든 그냥 타이핑했든 해석됩니다.
- **위키링크(내 노트).** `[[다른 노트]]`, `[[노트#섹션]]`,
  `[[#이 노트의 헤딩]]`, `![[임베드]]`를 따라가 읽습니다. 그래서
  "내가 [[X]]에서 뭐라고 결론냈지?"가 제목이 아니라 X의 내용으로 답해집니다.
  첨부파일은 건너뜁니다 — 이미지는 글이 아닙니다.

여기서 네트워크로 나가는 것은 없습니다. 해석하지 못한 포인터는 버리지 않고
이름이라도 남기므로, 답변이 무엇에 닿지 못했는지 말합니다. 지어내지 않습니다.

**긴 문서를 읽을 때.** popover는 문서의 앞부분이 아니라 **지금 있는 부분**을
가져갑니다. 책의 목차는 현재 페이지 기준으로 잘라내고, 긴 노트는 선택 영역과
질문을 기준으로 잘라냅니다. 빠진 부분이 있으면 프롬프트가 그 사실을 밝힙니다.
...
- **본인이 쓴 프로젝트 노트를 참조합니다 (v0.57.0)**: `01_Workspaces/<프로젝트>/`
  안에서 작업하며 사이드바에 질문하면, 어시스턴트가 그 프로젝트에 쓴 노트도 함께
  살펴보고 이미 내린 결론을 사용자 것으로 밝혀 제시합니다.

  범위에 대해 세 가지:

  - **노트는 색인되지 않습니다.** workspace의 어떤 것도 지식 그래프, 데이터베이스,
    `.curator/`에 들어가지 않습니다. 질문하는 그 순간에만 읽고 그 외에는 읽지
    않으므로, 프로젝트의 작업 상태는 프로젝트 안에 남습니다.
  - **지금 있는 프로젝트만.** workspace 밖에서 물으면 아무것도 참조하지 않습니다.
    vault 전체를 뒤지지 않고, 다른 프로젝트의 노트도 꺼내지 않습니다.
  - **에이전트 자신의 파일은 제외됩니다.** workspace 안의 `.agents/`,
    `CLAUDE.md`, `AGENTS.md`, `GEMINI.md`는 도구에 대한 지시이지 사용자의 생각이
    아닙니다. 어시스턴트가 자기 계획서를 "당신이 쓴 노트"라고 인용하는 건 아무 말도
    안 하는 것보다 나쁩니다.

  노트는 확립된 사실이 아니라 *사용자의 작업 노트*로 모델에 전달됩니다 — 그래서
  X라고 단정하기보다 "당신은 X라고 결론지었다"라고 말해야 합니다.

- **인용이 가리키는 논문으로 해석됩니다 (v0.56.0)**: `[8]`이 포함된 구절을 선택하고
  질문하면, 어시스턴트는 이미 `[8]`이 가리키는 참고문헌 항목을 갖고 있습니다. 추가
  왕복도, 참고문헌 절로 스크롤하라는 안내도 없습니다.

  참고문헌 절은 문서 끝에서부터 거슬러 올라가며 찾고, 여러 페이지에 걸쳐 따라갑니다.
  참고문헌은 heading을 한 번만 찍고 두세 페이지 더 이어지기 때문에, heading 페이지에서
  멈추면 대부분을 놓칩니다.

  **참고문헌과 매칭되지 않는 대괄호는 조용히 무시합니다.** `[8]`은 각주(`[^8]`),
  마크다운 참조 링크(`[text][8]`), 배열 인덱스(`arr[8]`)를 쓰는 방식이기도 해서
  중의적입니다. 추측하는 대신, 매칭되는 항목이 없는 번호는 "인용이 아님"으로 보고
  아무 말도 하지 않습니다. 인용이 전혀 없는 선택은 참고문헌 절을 읽지도 않으므로,
  코드에 대해 질문해도 비용이 들지 않습니다.

- **각 답변이 어디서 왔는지 (v0.56.0)**: 질문에 조회가 필요했던 경우, 답변 아래에
  무엇을 참조했는지 조용한 한 줄이 나타납니다 —
  `Eq. (29) — p.11 · [8] — Bartoli et al., 2005`.

  어시스턴트가 실제로 가져온 것에서 만들어지며, 답변을 되읽어서 추출하지 않습니다.
  그래서 쓰이지 않은 출처를 주장할 수 없습니다. 도달하지 못한 pointer는 **"not
  retrieved"**로 표시되며, "논문에 없음"이라고 하지 않습니다. 드래그 선택 popover는
  이미 로드된 페이지만 검색하므로, 못 찾았다는 것은 검색에 대한 진술이지 문서에 대한
  진술이 아닙니다. 조회가 필요 없었던 질문에는 이 줄이 아예 나타나지 않습니다.

- **텍스트 레이어에 없을 때 (v0.55.0)**: 일부 PDF는 본문에 표시되는 수식과 그림을
  이미지로 렌더링하기 때문에, 그 라벨이 추출된 텍스트에 아예 나타나지 않아 아무리
  *텍스트*를 검색해도 찾을 수 없습니다. 이제 어시스턴트는 페이지를 읽는 대신 볼 수
  있습니다. 가져온 텍스트에 질문한 수식, 그림, 스캔된 삽입물이 없으면, 그 페이지를
  렌더링해서 수동 스니핑과 동일한 추출 모델로 픽셀을 읽습니다. 한 번도 스크롤하지
  않은 페이지도 포함되고, 어떤 텍스트 레이어에도 담기지 않는 직접 손으로 쓴 여백
  메모도 포함됩니다.

  알아둘 만한 두 가지 결과:

  - **수식을 물어보려고 스니핑할 필요가 더 이상 없습니다.** "equation 29"에 대해
    물으면 어시스턴트가 페이지를 가져오고, 그 수식이 텍스트에 없다는 것을 알아채고,
    페이지 이미지에서 읽어냅니다.
  - **의도적으로 제한됩니다.** 페이지를 렌더링해서 vision 모델로 보내는 것은 텍스트를
    읽는 것보다 훨씬 비싸므로, 질문 하나당 적은 수의 페이지 이미지만 사용합니다.
    어시스턴트는 실제로 픽셀이 필요한 페이지에 그것을 쓰고 나머지는 텍스트로
    답변합니다.
  - **어디서 질문했는지에 따라 도달 범위는 여전히 달라집니다.** 드래그 선택 popover는
    이미 로드된 페이지에서 동작하고, 문서 전체 검색은 백엔드가 실행 중이어야 합니다.
    페이지를 이미지로 읽는 것은 이미 가진 페이지로 할 수 있는 일을 넓혀줄 뿐,
    각 표면이 애초에 어떤 페이지를 찾아볼 수 있는지를 바꾸지는 않습니다.

  텍스트를 찾지 못한 pointer는 `<unresolved_cross_references>` 블록에 나열됩니다.
  그 블록은 사용자가 아니라 어시스턴트에게 하는 말입니다. 이미지로 렌더링된 대상은
  논문에 없는 것이 아니라 텍스트 레이어에만 없는 것이니, 그보다 못한 답으로
  물러서기 전에 그 페이지를 이미지로 읽으라고 지시합니다. 페이지를 읽는 도구를 쓸 수
  없을 때만 주변 자료가 그 대상에 대해 establish하는 내용으로 설명하도록 물러섭니다.
  어느 쪽이든 사용자가 보는 답변은 어시스턴트가 무엇을 받았는지에 대한 보고가 아니라
  논문에 대한 것이어야 합니다.
...
> 데 세 번 걸렸습니다 (v0.56.1).**
>
> v0.53.1 전까지 기록하던 규칙은 `$read_file$()` 형식이었는데, Antigravity가
> 인식하지 못하는 형태입니다. 인식하지 못하는 규칙은 잘라내고 비어 버린
> `permissions` 객체를 통째로 삭제하므로, 이 권한은 **단 한 번의 실행도** 살아남지
> 못했습니다. v0.53.1은 이를 `read_file()`로 고치고 규칙이 파일에 *남아 있는지*를
> 확인했습니다. 실제로 남아 있긴 하지만 아무것도 허가하지 않습니다. 그래서 제대로
> 설정된 것처럼 보이는 규칙 뒤에서 이미지 읽기는 세 릴리스 더 자동 거부되었습니다.
>
> agy 1.1.13으로 실측한 결과, `read_file(*)`만 동작합니다. 정확한 경로
> (`read_file(/path/to/file.png)`)도 거부되고 `read_file(/tmp/*)`도 거부됩니다.
> 읽기에서 경로로 좁힌 규칙은 더 좁은 허가가 아니라 허가가 아닙니다.
>
> 사용자가 설정을 잘못한 것이 아닙니다. 업데이트하면 규칙이 다시 기록되고 죽은
> 규칙은 제거됩니다. 파일을 직접 고칠 필요는 없습니다.

> [!WARNING]
> **`read_file(*)` 의 대가를 그대로 적습니다.**
>
> 이 규칙은 Antigravity CLI가 **사용자 계정이 읽을 수 있는 모든 파일**을 읽게
> 허용합니다 — Incurator가 건네주는 페이지 이미지만이 아닙니다. agy는 더 좁은
> 형태를 받지 않고(정확한 경로도 거부), Incurator가 CLI를 감싸는 OS 샌드박스는
> *쓰기*만 막지 읽기는 막지 않습니다. 규칙이 망가져 있는 동안은 문이 우연히 닫혀
> 있었던 것이고, 이미지 읽기를 동작시키면 그 문이 열립니다.
>
> 이 권한은 **전역이고 지속됩니다**: `~/.gemini` 아래 파일 하나이고, 이후 그 계정의
> 모든 `agy` 실행이 이를 따릅니다. 여기에는 `wiki` 수집 파이프라인도 포함되는데,
> 그건 사용자가 쓰지 않은 PDF와 웹페이지를 처리하는 부분입니다. 수집된 문서 안에
> 심어진 지시가 모델에게 무관한 파일을 읽게 시킬 수 있고, 그 내용이 지식 베이스에
> 들어갈 수 있습니다.
>
> 허용되지 *않는* 것도 중요합니다. Incurator가 추가하는 규칙은 정확히 두 개이고,
> Antigravity는 headless에서 승인되지 않은 도구를 자동 거부하므로 CLI는 여전히
> 파일을 쓰거나 임의의 셸 명령을 실행할 수 없습니다. 현실적인 최악은 비밀 정보가
> 본인 vault로 읽혀 들어가는 것이지, 어딘가로 조용히 전송되는 것이 아닙니다.
>
> **이 거래를 아예 피하려면** PDF 추출을 API로 접근하는 vision 모델로 지정하세요
> (Dashboard → LLM Provider → PDF/LaTeX 추출 모델). API 프로바이더는 이미지 바이트를
> 직접 받으므로 파일시스템 권한이 개입하지 않고, 이 규칙도 필요 없습니다.
...
정상 답변에 로그인 안내가 인용돼 있어도 로그인 오류로 바뀌지 않습니다. 취소한
요청은 취소 상태를 유지하며, 완료되지 않은 native 응답은 미완료로 표시합니다.
Popover 문서 준비 중 근거 대기 시간이 지나더라도, 준비를 마칠 때 이미 도착한
근거는 추가 대기 없이 포함합니다.
...
## 15. 폴더 접근(Access) 탭 — Incurator가 파일을 읽지 못할 때

대시보드를 열고(리본 아이콘 또는 **Open Incurator Dashboard**) **Access** 탭을
선택하세요. Incurator가 읽어들이는 모든 폴더를 나열하고, 각 폴더를 실제로 읽을
수 있는지 알려줍니다.

**macOS에서는** 파일 접근 권한이 Obsidian이 아니라 **백엔드 프로세스**에
부여됩니다. 따라서 Finder에서 잘 보이고 더블클릭하면 잘 열리는 폴더인데도
여기서는 읽지 못할 수 있습니다. 이 탭은 바로 그 상황을 설명하기 위해
존재합니다. 예전에는 이런 경우가 아무 이유 없이 "PDF 파싱 실패"로만
나타났습니다.

**Linux에서는** 같은 판정이 일반적인 파일시스템 권한 문제를 뜻합니다. 백엔드를
실행하는 사용자 계정이 그 폴더를 읽을 수 있어야 합니다. 권한 부여 대화상자가
없으므로 버튼은 **Open folder…** 로 표시되며, 권한 프롬프트를 약속하는 대신
확인할 수 있도록 폴더를 열어줍니다.

이 탭은 **백엔드**가 실행 중인 시스템에 맞춰 안내 문구를 고릅니다.

각 행은 네 가지 판정 중 하나를 보여줍니다:

| 배지 | 의미 | 할 일 |
|---|---|---|
| `OK` | Incurator가 읽을 수 있음 | 없음 |
| `DENIED` | 폴더는 있지만 백엔드에 읽을 권한이 없음 | **Grant access…** 누르기 |
| `MISSING` | 볼트 자체가 Incurator가 기대하는 위치에 없음 | 볼트 경로 확인 |
| `NOT DOWNLOADED` | iCloud 등 클라우드 파일의 실제 내용이 내려와 있지 않음 | 파일을 한 번 열어 클라우드가 내려받게 하기 |

설정한 적 없는 폴더는 아예 표시되지 않습니다. 이 탭은 목록이 아니라 문제만
보여줍니다.

### 폴더 권한 부여하기

`DENIED` 행에는 버튼이 붙습니다 — macOS에서는 **Grant access…**, 그 외
플랫폼에서는 **Open folder…** 입니다. 이 버튼은 조치가 필요한 폴더를 여는데, 그 폴더가 행에 표시된 폴더와 같지 않을 수 있습니다. PDF가
`~/Library/Mobile Documents/com~apple~CloudDocs/Zotero`에 있다면 macOS가 실제로
허용을 요구하는 폴더는 `~/Library/Mobile Documents`이고, 버튼은 그 폴더를
엽니다.

그다음 macOS 프롬프트에 응답하고 **Re-check**를 누르세요.

재검증이 별도 버튼인 데는 이유가 있습니다. 사용자가 시스템 대화상자를 언제
끝냈는지 Incurator는 알 수 없으므로, 자동으로 검사하면 프롬프트가 아직 화면에
떠 있는 동안 "still denied"라고 잘못 보고하게 됩니다.

### 그래도 denied라고 나오면

Linux에서는 이것이 파일시스템 권한 문제이지 놓친 프롬프트가 아닙니다. 폴더의
소유자와 모드를 확인해 백엔드를 실행하는 사용자가 읽을 수 있도록 만든 뒤
**Re-check**를 누르세요.

macOS에서는 시스템이 권한을 Incurator 백엔드가 아니라 Obsidian에 부여했을 수 있습니다. 즉
Obsidian은 읽을 수 있게 됐지만 정작 권한이 필요한 프로세스는 여전히 읽지
못하는 상태입니다. 탭은 권한 부여가 실패한 척하지 않고 이 사실을 그대로
알려줍니다.

해결 방법은 백엔드를 실행하는 프로그램(보통 터미널)에 **시스템 설정 → 개인정보
보호 및 보안 → 전체 디스크 접근 권한**을 부여한 뒤 **Re-check**를 다시 누르는
것입니다.

### 명령줄에서 확인하기

Obsidian 없이 같은 보고서를 볼 수 있습니다:

```bash
wiki plugin access
```

각 루트마다 `state`와, 해당되는 경우 허용할 `grant_folder`를 담은 JSON을
출력합니다. 백엔드를 실행하는 것과 같은 사용자로 실행하세요 — 판정은 프로세스에
따라 실제로 달라지며, 그것이 바로 이 기능의 핵심입니다.
