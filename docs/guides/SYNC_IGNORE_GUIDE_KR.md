...
> 채팅 히스토리(`.curator/sessions.json`)는 안전한 보존과 동기화를 위해 플러그인 폴더 외부(`.curator/`)에 저장되며 세션 단위 병합, 삭제 tombstone, atomic replace를 지원합니다. Canonical file이 corrupt 또는 unreadable이면 원본을 보존하고 일반 save를 차단하며, missing이나 빈 default로 취급해 덮어쓰지 않습니다.
