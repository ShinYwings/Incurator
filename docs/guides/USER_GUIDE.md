...
#### 📂 저장소(Vault) 디렉토리 구조
`wiki init` 명령을 실행하면 InCurator는 지식 관리를 위해 다음과 같은 구조를 초기화합니다. 인큐레이터는 인간을 위한 공간(Root)과 기계 및 에이전트를 위한 공간(`.curator/`)을 엄격히 분리하여 관리합니다.
...
├── .curator/          # [Machine Space] 시스템 핵심 데이터 및 SQLite DB
...
### 2단계: 지식 추출 및 등록 (Ingest)
파일 배치가 끝났다면, Curator가 해당 파일을 읽고 지식 계층(L1~L3)으로 컴파일하도록 명령합니다.
...
이 명령을 통해 Curator는 원본 데이터를 파싱하여 **L1 요약, L2 원자적 사실 추출, L3 개념 연결** 작업을 자동으로 수행하며, 그 결과는 `.curator/` 내부에 기계 친화적인 형태로 저장됩니다.
...
#### 🧠 지식 밀도 (Collections)
파이프라인을 거쳐 정제된 '지식의 성숙도'를 나타냅니다.
-   **L1 Contexts**: 원본의 단순 요약본 개수입니다. 수집된 소스 수와 일치해야 합니다.
-   - **L2 Atoms**: 추출된 '원자적 사실'의 총량입니다. 이 숫자가 클수록 지식 베이스의 정보 밀도가 높음을 의미합니다.
-   - **L3 Concepts**: 사실들이 엮여 만들어진 '개념'의 개수입니다. 지식들 간의 연결망이 얼마나 촘촘한지 보여줍니다.
-   - **L4 Exhibitions**: 실제 질문에 답변하기 위해 최종 합성된 '전시물'의 개수입니다. 이 숫자는 사용자가 시스템을 얼마나 활발히 활용했는지 보여주는 지표가 됩니다.

> [!TIP]
> **성숙도 진단**: 만약 L1 수치에 비해 L2, L3 수치가 지나치게 낮다면, 아직 충분한 큐레이션(`wiki curate`)이 이루어지지 않은 상태일 수 있습니다. 더 깊이 있는 답변을 원한다면 큐레이션 파이프라인을 더 자주 가동해 보세요.
