...
| `wiki status` | Checks vault health and statistics. When this vault uses the Antigravity provider, it also reports when the CLI cannot reach the vault's tools — no MCP registry, the curator server missing from it, a registered command that cannot start, a registration pointing at a different vault, or the missing `mcp(*)` permission — naming which one, because the symptom you would otherwise see is the assistant reaching for a shell command and being refused, three steps removed from the cause. Reports a **Search index** warning when the vault holds knowledge units that search cannot find — a gap nothing surfaced before v0.64.0, and one that was 61% of the corpus when it was first measured. Silent when the index is complete. Also warns when the **local database is empty but the vault still holds a sync journal** — the database is machine-local, so clearing the repo cache, moving to a new machine, or **renaming the vault** mints a fresh empty one; the warning names the journal and the `wiki db import` command that restores it. | Checking overall system health |
...
-   **Search index degradation**: If embeddings, query expansion, or reranking are unavailable, lexical FTS5 search remains usable. A missing index records `vector_unavailable`; a runtime query-embedding error, invalid result, or query/index dimension mismatch records `vector_failed`; and invalid reranker output records `reranker_failed` while preserving the complete RRF order. Vector-only queries report the vector failure without inventing lexical hits. Corpus embedding and reranking accept only exact-cardinality finite numeric provider output, and all ready embeddings for one provider/model must share one dimension. Run `wiki reindex` after fixing provider/model configuration to rebuild chunks and embeddings.
