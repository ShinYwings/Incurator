...
- **부분 빌드는 없습니다**: 중간에 실패한 컴파일은 아무것도 발행하지
  ...
  provider가 rate limit으로 거부하면 job을 **즉시 재시작하지 않습니다.** 큰 소스를
  재시작하면 거부당한 단계에 도달하기도 전에 예산을 통째로 다시 씁니다 — 673페이지
  책에서 두 번 측정했고, 매번 약 90분이 폐기됐습니다. 대신 큐를 그대로 두고
  `wiki jobs run`이 얼마나 기다리면 되는지 알려줍니다.
