...
- **답변이 어떤 렌즈로 만들어졌는지 확인할 수 있습니다.** 워크스페이스 안에서
  질문하면 그 워크스페이스의 `curate.yml`이 답변이 참조할 수 있는 범위를 좁힙니다
  — 어떤 폴더, 어떤 검색 경로, 어떤 구분을 유지할지. 이제 context pack이 근거와
  함께 그 정책을 보고하며, **실제로 좁혔는지 여부**도 포함합니다. 설정은 돼 있지만
  아무것도 걸러내지 않는 렌즈가 추론이 아니라 그대로 드러납니다. vault 페르소나도
  함께 전달되므로, 답변을 쓰는 어시스턴트가 이 vault가 설정된 어조를 압니다.
