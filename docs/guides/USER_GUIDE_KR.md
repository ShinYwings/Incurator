...
**질문이 아닌 메시지에는 근거를 붙이지 않습니다.** 답변에 저장된 지식이 필요
없다고 분류되면 — "이 문단 번역해줘: …", "아래 글 요약해줘", "이거 다시 써줘" —
검색을 건너뛰고 메시지만으로 답합니다. 결과에는
`no retrieval: not a knowledge question` 경고가 붙습니다. 의도된 동작입니다:
번역 요청으로 볼트를 뒤지면 아무도 하지 않은 질문에 대한 근거를 고르게 되고,
검색어가 비면 사용자의 메시지 자체로 폴백하므로 붙여넣은 본문 전체를 검색하게
됩니다.

분류는 **이미 영어가 아닌** 질문에만 실행됩니다(위에 설명한 비용 문제). 따라서
영어로 쓴 "translate this: …"를 `wiki query`, MCP 도구, `wiki plugin query`에
보내면 여전히 평소처럼 검색됩니다. 팝오버와 사이드챗은 언어와 무관하게 모든
메시지를 분류하므로 이 간극의 영향을 받지 않습니다.
...
- **부분 빌드는 없습니다**: 중간에 실패한 컴파일은 아무것도 발행하지
  ...
  **L2 추출은 재개 가능합니다 (v0.62.0).** 멈추기 전에 끝난 배치들은 보존되므로,
  ...
  있는 범위를 넓히지 않으며, native output도 모델의 도구 요청을 차단하지는
  않습니다. agy가 도구를 거부하고 답변을 반환하지 않으면 CLI가 SUCCESS라고
  해도 backend는 거부 오류로 전달합니다. 그래프 추출은 기존 재시도 한도 안에서
  원래 배치를 다시 요청하고, 이전에 검증된 배치를 재사용합니다. 복구된 답변과
  유효한 빈 추출 결과는 그대로 수용하며, 불완전한 그래프는 발행하지 않습니다.
...
**pack은 자신을 만든 curation lens를 밝히고 vault 페르소나를 함께 싣습니다**
(v0.75.0). 워크스페이스 안에서 질문하면 그 워크스페이스의 `curate.yml`이 답변이
참조할 수 있는 범위를 좁힙니다 — 어떤 폴더, 어떤 검색 경로. pack은 그것을
`policy.applied_filters`로 보고하며, **빈 목록은 렌즈가 돌았지만 아무것도 좁히지
않았다는 뜻**입니다. 설정은 돼 있지만 작동하지 않는 렌즈가 근거로부터 추론하는
대신 그대로 드러납니다.

실제로 검색을 좁히는 설정만 필터로 표시됩니다. 워크스페이스가 선언한 나머지는
`policy.declared`에 들어가며 — 무엇을 요청했는지는 알려주되 시스템이 그에 따라
동작했다고 주장하지는 않습니다. 검색기가 항상 허용하는 경로는 워크스페이스가
제외했더라도 허용된 것으로 보고합니다. 그러지 않으면 좁힘 정도를 과장하게 됩니다.

`persona`가 함께 전달되므로, 답변을 쓰는 어시스턴트가 이 vault가 설정된 어조를
압니다.
...
| `wiki status` | 저장소 상태 및 통계를 확인합니다. 볼트가 보유하고 있지만 검색이 찾을 수 없는 knowledge unit이 있으면 **Search index** 경고를 표시합니다 — v0.64.0 이전에는 어디에도 드러나지 않던 격차이고, 처음 측정했을 때는 코퍼스의 61%였습니다. 인덱스가 온전하면 아무것도 출력하지 않습니다. 또한 **로컬 데이터베이스가 비어 있는데 볼트에 sync journal이 남아 있으면** 경고합니다 — 데이터베이스는 machine-local이라 repo 캐시를 지우거나, 새 기기로 옮기거나, **볼트 이름을 바꾸면** 새 빈 DB가 만들어집니다. 경고는 해당 journal과 복구용 `wiki db import` 명령을 함께 알려줍니다. 볼트가 Antigravity provider를 쓰는 경우, CLI가 이 볼트의 도구에 닿지 못하는 상태도 보고합니다 — MCP 레지스트리 없음, 레지스트리에 curator 서버 없음, 등록된 명령이 실행 불가, 다른 볼트를 가리키는 등록, `mcp(*)` 권한 누락 중 무엇인지 지목합니다. 그러지 않으면 보이는 증상은 어시스턴트가 셸 명령을 시도했다가 거부되는 것뿐이라 원인에서 세 단계 떨어져 있습니다. | 전반적인 건강 상태 체크 시 |
...
| `wiki source dedupe-paths` | 유니코드 정규화 형태만 다른 소스들을 병합합니다. 기본은 보고만 하고, `--apply`를 줘야 실제로 씁니다. | macOS와 다른 OS 사이에서 동기화한 뒤 같은 파일이 두 번 등록된 경우 |
...
| `wiki gc run` | 회수 가능한 항목, 보관 기간이 지난 채팅 세션, 그리고 `gc.prompt_runs_keep` 상한을 넘은 LLM 호출 기록을 삭제합니다 — 채팅 삭제는 **동기화되는 모든 기기에 적용되며 되돌릴 수 없다**고 확인 창에서 명시합니다(먼저 확인을 요청). 비어 있음이 확인된 임시 vault 캐시만 대상입니다: 해석된 vault 경로가 임시 루트 아래에서 사라졌고, 캐시에는 경로 마커와 선택적으로 인식 가능한 빈 DB만 있으며, 검사 결과 보존할 애플리케이션 행이 없어야 합니다. Tombstone, 진단 기록, 작업 이력, 알 수 없는 스키마/파일, 백업 및 기존 SQLite sidecar가 있으면 캐시를 보존합니다. 검사 중 후보 DB를 초기화하거나 마이그레이션하지 않으며, 삭제 직전에 파일시스템 식별 정보와 적격 조건을 다시 확인합니다. 임시 경로가 아닌 사라진 경로는 연결 해제된 드라이브일 수 있으므로 대상이 되지 않습니다. |
...
캐시 검사는 논리 SQLite 테이블을 확인합니다. 초기화된 FTS shadow 메타데이터와
스키마 버전 표시는 사용자 레코드로 세지 않으며, source가 0개라는 사실만으로는
충분하지 않습니다. 미리보기 재검증은 확인 대기 중 관찰된 변경을 거부하지만,
마지막 검사 이후 쓰기를 시작하는 프로세스를 배제하는 독점 잠금은 아닙니다.
보관 설정을 저장하는 것만으로 GC가 실행되지는 않습니다.
...
- **하나의 주장에 하나의 엣지.** 두 기기가 같은 두 대상 사이의 같은 관계를 각자
  추출해도 한 번만 저장됩니다(v0.74.0). 엣지가 중복되면 단순히 지저분한 게 아니라,
  그래프는 traversal이 걷고 community 요약이 세는 대상이므로 이후 모든 질문에서
  두 번 가중됩니다. 같은 쌍에 대해 *다른* 내용을 말하는 두 주장은 그대로 둘 다
  남습니다 — 관계 유형이 엣지를 구분하는 요소의 일부입니다.
