...
- v0.2.0 documentation:
  - `docs/plans/INCURATOR_SYSTEM_BUILD.md`
...
- `backend/pyproject.toml` now uses `readme = "../README.md"`.
...
- `backend/src/curator/cli.py` benchmark path offsets now account for the
  backend directory depth.
...
git diff --check -- setup.sh backend plugin scripts docs README.md README_EN.md AGENTS.md CLAUDE.md GEMINI.md
```
...
git diff --check -- backend/src/curator backend/tests docs setup.sh plugin scripts AGENTS.md CLAUDE.md GEMINI.md README.md README_EN.md
```
