...
`docs/plans/db_metadata_sync_plan.md`에서 계획된 `sync_metadata_to_db()`:
...
`docs/plans/orchestrated_pipeline_and_persona_system.md`의 Phase 3에서
...
**Phase 4 Benchmark Harness (`scripts/benchmark/benchmark.py`)는 v0.2.1 범위에서 제외.**
구현 비용 대비 우선순위가 낮으므로 v0.2.2 이후로 연기.
