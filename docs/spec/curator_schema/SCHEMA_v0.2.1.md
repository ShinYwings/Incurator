# Incurator - Schema & Operating Conventions (v0.2.1)

Audience: Incurator backend, Obsidian plugin, MCP clients, and coding agents.

This document is the schema source of truth for the v0.2.1 line. Implementation
plans under `docs/plans/update_plan/` may explain sequencing and alternatives,
but code must conform to this file when it writes `.curator/` state, Curator DAG
pages, MCP payloads, or provider/model configuration.

v0.2.1 extends v0.2.0. Any v0.2.0 field not contradicted here remains valid.
...
`gemini-cli` is deprecated. Existing config values may be accepted as a
backward-compatible alias, but new writes must use `antigravity-cli`.
...
Legacy `gemini_flash_model` and `gemini_think_model` may be read as migration
fallbacks. They must not be the canonical field names in new docs or new config
writes.
...
v0.2.1 L1 Contexts preserve structural source text for immediate RAG and later
...
- Legacy `gemini-cli` config may be read, but `antigravity-cli` is canonical.
