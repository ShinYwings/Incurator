# Incurator - Schema & Operating Conventions (v0.3.1)

Audience: Incurator backend, Obsidian plugin, MCP clients, and coding agents.

This document represents the most concrete layer (`spec`) of the documentation hierarchy (`philosophy` -> `guides` -> `spec`). It is the absolute schema source of truth for the v0.3.1 line.
...
v0.3.1 extends v0.2.2. Any v0.2.2 field not contradicted here remains valid. Sections 1–10 below preserve the v0.2.2 schema contract verbatim; the v0.3.1-specific schema additions (source spans, typed knowledge units, graph entities/relations, community reports, memory paths, prompt runs, curation plans, insight candidates, and artifact dependencies) are defined in sections 11 onward. The archived v0.2.2 source of truth lives at `docs/specs/curator_schema/archives/SCHEMA_v0.2.2.md`.

**Clean-rebuild stance (no migration compatibility shims).** v0.3.1 is a
curation-native rebuild that does **not** maintain backward-compatibility shims
for the v0.3.1 migration itself. There are no prompt-function wrapper layers, no
legacy-query fallback path, and no `curate.yml` persona→KRS auto-mapping. The new
shapes below are the only supported shapes; an existing vault is expected to be
rebuilt (`wiki reset` + `wiki add` + `wiki build`) rather than silently migrated.
...
# v0.3.1 Curation-Native Schema Additions

The sections above (1–10) define the inherited v0.2.2 contract. The sections
below define the new schema introduced by the v0.3.1 curation-native rebuild.
...
`docs/specs/system_behavior/SYSTEM_BEHAVIOR_v0.3.1.md`; the plugin contract is
`docs/specs/plugin_schema/PLUGIN_SCHEMA_v0.3.1.md`. All three must stay
version-synchronized.

### Storage model: DB is the single source of truth; markdown is a derived projection

v0.3.1 fixes the storage model so the system behaves like a compiler:
...
- **The `.curator/Collections/` L1–L3 markdown pages (`CTX-`, `ATM-`, `CON-`) are
  a DERIVED, disposable projection of the DB ("object code").** They are emitted
  from DB records and are NOT authoritative. Their sole purpose is to let `qmd`
  index them for high-quality hybrid (BM25 + vector) search without rewriting the
  search engine. They may be deleted and re-emitted from the DB at any time.
  Because they are emitted (never edited as truth), there is no DB↔file drift.
- **L4 Exhibitions are the only human/agent-facing artifact.** They are emitted to
  the Human-Only Space (`02_Wiki/`, see ABOUT.md), and editing an Exhibition is
  reverse-parsed back into the DB (backprop).
...
- `classification` follows the backprop lifecycle in
  `SYSTEM_BEHAVIOR_v0.3.1.md`. A `derived_insight` candidate must never be written
...
semantics live in `SYSTEM_BEHAVIOR_v0.3.1.md` §16 and the user guides.
