...
## 13.1 Syncthing Auto-Sync (Export/Import)

Incurator supports P2P DB synchronization for `state.sqlite` via Syncthing.
Instead of relying on a central server, the backend relies on a transport file
(`.curator/sync.jsonl`) generated by LWW (Last-Write-Wins) export.

Rules:
- The Obsidian plugin automatically invokes a silent DB Export to `sync.jsonl` whenever a user saves or modifies a file, ensuring the local state is always prepped for Syncthing.
- The Obsidian plugin automatically invokes a silent DB Import from `sync.jsonl` when the workspace is loaded (e.g., computer turns on or Obsidian opens).
- **Infinite Loop Prevention**: Import operations must never trigger an Export. The backend must track the last processed `sync.jsonl` hash or timestamp in `.curator/sync_meta.json`. If `import_knowledge` is called with a file identical to the last imported state, it must no-op without firing `Vault.on('modify')` equivalents.
- **Clock Skew Defense**: During Import, if the maximum `updated_at` in the incoming `sync.jsonl` is significantly in the future compared to the local device's clock, the system must warn the user of potential data loss due to clock skew, as LWW assumes accurate timestamps.
- The Obsidian plugin must provide a manual "Sync DB" Ribbon action for explicit overrides.
