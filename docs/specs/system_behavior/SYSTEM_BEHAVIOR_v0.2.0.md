# Incurator - Agentic Zettelkasten Curator (v0.2.0)

Incurator is an autonomous, AI-maintained personal knowledge base designed for the Incurator v0.2.0 architecture. It functions as a Multi-Agent DAG (Directed Acyclic Graph) RAG system built to manage fragmented knowledge assets and execute complex projects safely without hallucinations.
...
## 0. v0.2.0 Transition Status And Authority

This document is the target architecture contract for the v0.2.0 line. It is not allowed to hide transition work behind optimistic wording. When the implementation is still staged, the document must say so explicitly.

The authority split is fixed:

- **Incurator backend** is the source-of-truth system. It owns `.curator/state.sqlite`, source registration, source hashing, source provenance, L1-L4 DAG construction, QMD search, MCP tools, and any durable ingest or healing decision.
- **Obsidian plugin** is a client. It may observe open tabs, capture the current PDF viewer state, assemble prompt context, render chat UI, ask the human for import/rebind approval, and call backend MCP tools. It must not mutate `.curator/` directly.
- **Human-authored notes** in `03_Notes/` remain human truth. MCP tools and plugin automation must not edit them silently.
- **External resources** may be either copied into `04_Resources/` by explicit user-approved import, or tracked in Reference Mode without copying. The chosen policy must be visible to the user and recorded in backend state.

The repository layout is also in transition:

- The target monorepo layout is `backend/` for the Python engine and `plugin/` for the Obsidian client.
- Historical or current development checkouts may still expose backend code at repository root (`src/`, `tests/`, `pyproject.toml`) and the active plugin under a vault path such as `.obsidian/plugins/incurator-obsidian-agent`.
- Any migration from the historical layout to the target layout must preserve git history where practical, preserve build/test commands, and update every path assumption with evidence. The implementation plan for that work lives in `.agents/plans/2024-05_v0.2.0_system_build/INCURATOR_SYSTEM_BUILD.md`; the corresponding current-state ledger lives in `.agents/plans/2024-05_v0.2.0_system_build/INCURATOR_SYSTEM_BUILD_EVIDENCE.md`.

The plugin identity is no longer `obsidian-ai-agent`. The canonical plugin/client name for this architecture is `incurator-obsidian-agent`. Old paths and names may appear in historical notes, but new implementation and documentation should use the new name unless describing legacy migration.
...
## 1.4 Architecture Shift: Monorepo & Platform Isolation (v0.2.0)

### 1.4.1 The "Two-Repo" Friction (AS-IS vs TO-BE)
- **AS-IS (v0.1.0)**: The Python backend daemon and the Obsidian plugin were managed in completely separate GitHub repositories. Any feature requiring changes to both (e.g., adding a new MCP tool or changing the database schema) required coordinating commits across two repos, updating git submodules inside the user vault, and manually pushing built assets. This resulted in an agonizingly slow and fragile development loop.
- **TO-BE (v0.2.0)**: The target state is a single monorepo (`ShinYwings/Incurator`) containing the backend (`backend/`) and the Obsidian plugin (`plugin/`). Developers can then implement end-to-end features in a single Pull Request.
  - **Plugin Build Innovation**: We eliminated git submodules for the plugin deployment. Instead, the `esbuild` process uses the `OBSIDIAN_PLUGIN_DIR` environment variable to dynamically emit compiled files (`main.js`, `manifest.json`, `styles.css`) directly into the active Obsidian vault. This provides an instant "watch-mode" development experience without polluting the vault's git history.
  - **Transition requirement**: Until the monorepo move is complete, docs and scripts must distinguish target paths from active development paths. The active Obsidian plugin path may still be `/home/shin/Workspace/second_brain/.obsidian/plugins/incurator-obsidian-agent`; this must not be described as a backend source-of-truth directory.

### 1.4.2 Syncthing Path Collisions (AS-IS vs TO-BE)
- **AS-IS (v0.1.0)**: Absolute paths (like the path to the `wiki` binary or the Zotero library) were hardcoded inside the vault's `.obsidian` configuration. When syncing the vault across multiple operating systems (e.g., Linux desktop and macOS laptop) via Syncthing, these absolute paths caused constant collisions and broken links.
- **TO-BE (v0.2.0)**: We completely isolated machine-specific absolute paths from the synced vault.
  - **Global Configuration**: Machine-specific paths (like Zotero libraries) are now stored in `~/.config/curator/config.yml` (the user's home directory), escaping the Syncthing sync loop.
  - **OS Separation in UI**: The Obsidian plugin settings now explicitly separate `Linux Binary Path` and `macOS Binary Path`. The agent dynamically selects the correct executable based on the active operating system (`process.platform`).

## 1.5 Architecture Shift: External Resource Integration (v0.2.0)

### 1.5.1 The Zotero Duplication Problem (AS-IS vs TO-BE)
- **AS-IS (v0.1.0)**: To process an external PDF (e.g., a paper managed in Zotero), users had to physically copy the file into the `04_Resources/` folder of the vault. This duplicated massive files and, more importantly, broke the "Single Source of Truth". If a user annotated the PDF on their iPad using Apple Pencil (synced via iCloud/Zotero), the copy inside the vault became instantly outdated.
- **TO-BE (v0.2.0)**: Reference Mode becomes a first-class backend policy. External files may remain in their original location (e.g., the Zotero library). The Incurator Engine reads them directly, calculates their SHA-256 Content Hash, and generates a lightweight proxy resource note or source record inside the vault to serve as a stable LLM anchor. `state.sqlite` records `is_reference=1`, tracks the file via its Content Hash, and stores `external_path` only as a recoverable location hint.
  - **Copy mode remains valid**: Some users intentionally want a managed vault copy. In that case the backend imports the file into `04_Resources/`, never into `03_Notes/`, using a user-approved destination. The default destination mirrors the active note hierarchy: `03_Notes/Vision/Foo.md` proposes `04_Resources/Vision/Foo/<pdf-file>.pdf`.
  - **Client responsibility**: The Obsidian plugin may propose and confirm the destination. It must call backend tools for import/status/ingest and must not write Incurator source state on its own.

### 1.5.2 Hash Drift Defense & Healing (AS-IS vs TO-BE)
- **AS-IS (v0.1.0)**: If a copied PDF was modified or moved, the system either ignored the new content or duplicated it again, leading to knowledge fragmentation.
- **TO-BE (v0.2.0)**: If a user annotates an external PDF on their iPad, the file's hash changes (**Hash Drift**). If the file is moved in the Zotero library, its path breaks.
  - **Detection**: Incurator v0.2.0 detects these drifts when the `external_path` cache fails. The backend performs a high-speed `rglob` re-discovery scan on active roots (e.g., `external.zotero.path`). If unresolved, the backend returns `STATUS: MOVED`.
  - **Healing (Human-in-the-Loop)**: The Obsidian plugin presents a UI banner: *"Location moved. Accept re-binding?"*. Upon human acceptance, the `curator_rebind_source` tool updates the backend source record, any proxy anchor metadata used by the implementation, and the `logical_source_id` relationship, safely restoring the knowledge link without autonomous data manipulation.


## 2. Polymorphic Metadata Schema (L1-L4)

[See SCHEMA_v0.2.0.md for full details]
...
---
This specification defines the operational contract for Incurator v0.2.0.
