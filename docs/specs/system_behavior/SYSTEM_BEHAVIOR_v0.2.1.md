...
This document represents the most concrete layer (`spec`) of the documentation hierarchy (`philosophy` -> `guides` -> `spec`). It is the absolute behavior source of truth for the v0.2.1 line. It defines how the backend, plugin, MCP tools, and workspace agents interact. Schema details live in `docs/specs/curator_schema/SCHEMA_v0.2.1.md`.

Implementation plans under `.agents/plans/2024-05_v0.2.1_update/` are transient and strictly subordinate to this document. If a plan conflicts with this behavior contract, update the plan or bring the conflict back to review before coding.
...
The Incurator MCP server exposes over 40 tools. The following list highlights the *newly introduced tools specific to v0.2.1 behavior*. For a complete and exhaustive list of all MCP tools and their descriptions, please see [MCP_USER_GUIDE.md](file:///Users/shin/shinywings/Incurator/docs/guides/MCP_USER_GUIDE.md).

v0.2.1 introduces these new core MCP tools:
