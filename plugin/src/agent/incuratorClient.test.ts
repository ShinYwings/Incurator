...
/**
 * The sidechat job indicator vanished because it read
 * `<vault>/.curator/runtime/jobs.json` directly. Runtime snapshots moved to the
 * repo-local cache and nothing rewrites the vault-side copies, so on the
 * reporting vault that file froze at 2026-07-04 with `running: []` while the
 * live snapshot showed a job running — the indicator rendered nothing and
 * looked like it had been deleted.
 */
describe("getJobsSnapshot", () => {
  it("reads jobs from the live status payload", async () => {
    const calls: string[][] = [];
    const backendJson = async (args: string[]) => {
      calls.push(args);
      return {
        status: {},
        sources: {},
        jobs: { running: [{ job_id: 40 }], queued: [], idle: false },
      };
    };
    const client = new IncuratorClient(settings(), "0.48.2", backendJson);
  ...
  it("never reads the on-disk snapshot file", async () => {
    // The whole point: a path-based read is what froze. If someone
    // reintroduces one here, this fails.
    const source = IncuratorClient.prototype.getJobsSnapshot.toString();
    expect(source).not.toContain("runtime");
    expect(source).not.toContain("jobs.json");
  });

  it("returns null when the backend gives nothing usable", async () => {
    for (const payload of [null, {}, { jobs: null }, "not an object"]) {
      const client = new IncuratorClient(
        settings(),
        "0.48.2",
        async () => payload as unknown
      );
      expect(await client.getJobsSnapshot()).toBeNull();
    }
  });
});
