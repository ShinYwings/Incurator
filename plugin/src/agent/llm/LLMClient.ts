...
export class LLMClient {
  ...
  private syncAgyMcpConfig(): void {
    ...
    // v0.29.0: Write a read_file policy so headless agy (-p --sandbox) auto-
    // approves reads within the --add-dir roots instead of auto-denying.
    // The v0.23.0 removal of the blanket permission bypass left no tool-
    // level approval for read_file; the policy engine (.toml in ~/.gemini/
    // policies/) is the supported replacement. The OS sandbox
    // (wrapWithOsSandbox) remains the actual containment boundary.
    this.syncAgyReadPolicy();
  }

  /**
   * Write (or update) a policy .toml that allows the agy read_file tool for
   * the configured allowed roots. Placed in ~/.gemini/policies/ which the
   * policy engine auto-loads — no policyPaths registration needed.
   */
  private syncAgyReadPolicy(): void {
    const policyDir = join(homedir(), ".gemini", "policies");
    mkdirSync(policyDir, { recursive: true });
    const policyPath = join(policyDir, "incurator-read.toml");

    // Build one [[rule]] per allowed root so reads are scoped, not blanket.
    const roots = this.allowedRoots();
    const rules = roots.map(
      (root) =>
        `[[rule]]\ntoolName = "read_file"\ndecision = "allow"\npriority = 100\n# Scoped to: ${root}`,
    );
    // If no roots are configured, still allow read_file for the vault at
    // minimum — the agent needs to read files the user drags into the chat.
    if (rules.length === 0) {
      rules.push(
        `[[rule]]\ntoolName = "read_file"\ndecision = "allow"\npriority = 100\n# Fallback: no roots configured`,
      );
    }

    const content = `# Auto-generated by Incurator plugin — do not edit manually.\n# Allows agy read_file for the plugin's allowed roots (vault + Zotero).\n# The OS sandbox (Seatbelt / bwrap) is the actual containment boundary.\n\n${rules.join("\n\n")}\n`;
