...
// Antigravity PRUNES allow-rules it does not recognise, then drops an empty
// `permissions` object entirely. `$read_file$()` — the form shipped since the
// rule was introduced — is not a recognised rule, so it was silently deleted on
// the CLI's very next run and the grant survived exactly zero model calls.
//
// v0.53.1 corrected the SHAPE and verified the rule survived a run. Surviving
// is not granting, and that distinction is what let the bug live another three
// releases: `read_file()` persists in settings.json and authorizes nothing, so
// every image read still came back
//   "jetski: no output produced — a tool required the read_file permission
//    that headless mode cannot prompt for, so it was auto-denied".
//
// Measured against agy 1.1.13, each rule written to settings.json and then
// asked to read a real PNG:
//
//   read_file(*)                    -> the model read the file
//   read_file(/tmp/agyprobe.png)    -> auto-denied  (an EXACT path is refused)
//   read_file(/tmp/*)               -> auto-denied
//   read_file()                     -> auto-denied  (what v0.53.1 shipped)
//
// Only the wildcard is honoured — a path-scoped read rule is not a narrower
// grant, it is no grant. `command(wiki)` was measured the same way and DOES
// work scoped, so the v0.23.0 posture keeps its narrow command permission; only
// the read rule is forced wide by the CLI's own parser.
const AGY_READ_PERMISSION = "read_file(*)";
// Spawning the configured MCP server ("wiki mcp") needs the command permission.
// This became reachable in v0.53.0, when the Obsidian plugin first got an
// incurator MCP entry — before that the model had no curator tool to call, so
// the denial never fired on this path. SCOPED to `wiki` on purpose: a bare
// `command()` would be the blanket bypass the v0.23.0 posture forbids. Measured:
 ...
 * read access. Never includes a rule we did not write ourselves.
 */
const AGY_RETIRED_PERMISSIONS = ["read_file()", "$read_file$()"];
const INCURATOR_AGY_POLICY_HEADER =
  "# Auto-generated by Incurator plugin — do not edit manually.";

// ─── Embedded MCP Fetch Server ──────────────────────────────────
// A minimal, zero-dependency Node.js MCP stdio server that exposes a
// `fetch_url` tool (GET-only). Written to ~/.gemini/incurator/ at plugin
// startup and auto-injected into the agy settings.json so the headless CLI
// can fetch URLs without requiring a shell `curl` permission grant.
const INCURATOR_FETCH_MCP_KEY = "incurator_fetch";
export const FETCH_MCP_SERVER_SCRIPT = `#!/usr/bin/env node
"use strict";
...
rl.on("line", async (line) => {
  ...
  if (method === "tools/call") {
    ...
    if (!args.url || typeof args.url !== "string") {
      respond(id, {
        content: [{ type: "text", text: "Error: 'url' argument is required and must be a string." }],
        isError: true,
      });
      return;
    }
    try {
      ...
      const truncated = result.body.length > 100000
        ? result.body.slice(0, 100000) + "\\n[truncated at 100KB]"
        : result.body;
      respond(id, {
        content: [{ type: "text", text: "HTTP " + result.status + "\\n\\n" + truncated }],
      });
    } catch (err) {
      respond(id, {
...
export function syncAgyHeadlessReadPermission(geminiDir: string): void {
  ...
  const merged = {
    ...
    permissions: {
      ...permissions,
      allow: [
        // Drop rules THIS function shipped in an earlier release and has since
        // measured to be inert. `read_file()` persists in settings.json and
        // authorizes nothing, so leaving it behind on upgrade would keep a dead
        // grant sitting next to the live one, looking for all the world like
        // read access is configured. Only our own retired output is removed;
        // every rule the user or another tool added is preserved untouched.
        ...allow.filter((rule) => !AGY_RETIRED_PERMISSIONS.includes(rule)),
        ...AGY_REQUIRED_PERMISSIONS.filter((rule) => !allow.includes(rule)),
      ],
    },
  };
...
export class LLMClient {
  ...
  private syncAgyMcpConfig(): void {
    ...
    // Auto-inject the built-in fetch MCP server so the headless agy CLI can
    // fetch URLs without needing a shell `curl` permission grant. The script
    // is written to a deterministic path under ~/.gemini/incurator/ and only
    // injected when the user has not configured a server with the same key.
    if (!(INCURATOR_FETCH_MCP_KEY in mcpServers)) {
      const incuratorDir = join(geminiDir, "incurator");
      mkdirSync(incuratorDir, { recursive: true });
      const fetchScriptPath = join(incuratorDir, "fetch_mcp_server.js");
      const existingScript = existsSync(fetchScriptPath) ? readFileSync(fetchScriptPath, "utf-8") : "";
      if (existingScript !== FETCH_MCP_SERVER_SCRIPT) {
        writeFileSync(fetchScriptPath, FETCH_MCP_SERVER_SCRIPT);
      }
      mcpServers[INCURATOR_FETCH_MCP_KEY] = {
        command: "node",
        args: [fetchScriptPath],
      };
    }
