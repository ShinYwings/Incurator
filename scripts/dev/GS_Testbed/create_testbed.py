#!/usr/bin/env python3
"""Create the repo-local initialized testbed vault for incurator development."""
...
TESTBED_ASSETS = Path(__file__).resolve().parent   # scripts/dev/GS_Testbed/
TESTBED_STAGE = TESTBED_ASSETS / "stage"
FIXTURE_WORKSPACE_RULES = TESTBED_ASSETS / "fixture_workspace_rules"
...
def _ensure_testbed_stage(root: Path) -> None:
    ...
    if not TESTBED_STAGE.exists():
        raise SystemExit(f"Missing testbed stage: {TESTBED_STAGE}")
    shutil.copytree(TESTBED_STAGE, root, dirs_exist_ok=True)


def _copy_template_index(paths: cfg.WikiPaths) -> None:
    template_path = SRC_ROOT / "curator" / "workspace" / "templates" / "index.md"
...
This initialized testbed vault is generated by `scripts/dev/GS_Testbed/create_testbed.py`.
It is used for incurator development validation.
""",
    )
    _write(
        paths.ledger,
...
def _write_testbed_agent_rules(root: Path) -> None:
    ...
    if not FIXTURE_WORKSPACE_RULES.exists():
        raise SystemExit(f"Missing testbed workspace rules: {FIXTURE_WORKSPACE_RULES}")
...
def main() -> None:
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Recreate the testbed from the checked-in GS_Testbed/stage fixture.",
    )
