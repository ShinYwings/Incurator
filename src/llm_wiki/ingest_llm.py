"""LLM curate pipeline — global sequential DAG build.

Runs after `wiki add` (which generates L1 Contexts).
...
Global pipeline executed by ingest_pending():
    ...
    Phase C — ALL Concept files      → L4 Exhibitions (global synthesis)
    Phase D — rebuild index, log, ledger, overview

This ensures cross-source concepts and exhibitions emerge correctly.
All IDs are UUID-based (ATM-/CON-/EXH-).
"""
...
# ---------------------------------------------------------------------------
# v13.1 entry points — pipeline split (wiki add = L1-L3, wiki curate = L4)
# ---------------------------------------------------------------------------
