...
def add_file(
    ...
    """Add a single file to the tracking DB. Does not copy the file."""
    ...
    # Guard: must be inside one of raw_dirs but NOT in WIKI_DIR
    in_raw = _is_inside_raw(source, paths.raw_dirs)
    try:
        source.relative_to(paths.wiki.resolve())
        in_raw = False
    except ValueError:
        pass
    ...
    if not in_raw:
         return AddOutcome(
            ...
            source_path=source,
            relpath=str(source),
            message=f"File is outside the allowed raw directories or inside the distilled output directory: {source}",
        )
    ...
    try:
        ...
        return AddOutcome(
            ...
            relpath=str(final_path.relative_to(paths.root)) if _is_inside_raw(final_path, paths.raw_dirs) else str(final_path),
    ...
    with db.connect(paths.state_db) as conn:
        existing = conn.execute(
            "SELECT id, relpath, content_hash FROM sources WHERE relpath = ?",
            (relpath,),
        ).fetchone()
        ...
        if existing is not None:
            if existing["content_hash"] == parsed.content_hash:
                ...
                conn.execute(
                    "UPDATE sources SET content_hash = ?, bytes = ?, status = 'pending', last_ingested = NULL WHERE id = ?",
