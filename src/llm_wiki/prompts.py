...
Three passes:
    1. extract — read source, return structured JSON with extracted concepts
    2. draft_page — generate a single extracted page
...
1. Wiki pages use YAML frontmatter with these fields:
   ...
   - created: YYYY-MM-DD
   - updated: YYYY-MM-DD
   - sources: ["Metadata/source-slug.md"]
  ...
  "extracted": [
    {
      ...
      "type": "extracted",
      "description": "1-2 sentences describing this atomic concept or entity based on the source"
    }
  ],
  "tags": ["tag1", "tag2", "tag3"]
}
...
- Extract 2-15 atomic core concepts/entities (people, organizations, ideas, techniques, topics).
- Slugs must be kebab-case ASCII. Use the canonical name. For concepts use the shortest canonical form (rag, not retrieval-augmented-generation).
...
# ---------------------------------------------------------------------------
# Pass 2 — Draft a new extracted page
# ---------------------------------------------------------------------------

NEW_EXTRACTED_PAGE_TEMPLATE = """Draft a wiki page for the extracted atomic concept/entity '{name}'.

This concept was extracted from the source: '{source_title}' (Metadata/{source_slug}.md)
...
Related concepts also mentioned in this source (use [[wikilinks]] to connect to them):
{related}

Write a complete markdown page with:
1. YAML frontmatter (title, type: extracted, tags, created: {today}, updated: {today}, sources: ["Metadata/{source_slug}.md"], confidence)
2. An H1 heading matching the title
3. A 2-4 paragraph body (150-400 words) describing this concept:
   - What it is / Definition
   - Why it matters
   - How it relates to other concepts (use [[wikilinks]])
4. End with a '## Sources' section listing [[Metadata/{source_slug}]]
...
def build_draft_page_messages(
    ...
    """Pass 2 — draft a single new extracted page."""
    template = NEW_EXTRACTED_PAGE_TEMPLATE
...
4. Adding "Metadata/{source_slug}.md" to the 'sources:' list in frontmatter (keep existing entries).
5. Adding [[Metadata/{source_slug}]] to the '## Sources' section at the bottom.
...
SOURCE_PAGE_TEMPLATE = """Draft a metadata mapping page for the ingested document.
This page acts as the Wiki-Resource Mapping linking 03_Resources (or 02_Wiki) to .wiki/distilled.
...
Extracted atomic concepts created/updated from this source:
{extracted_links}
...
5. A 'Related Pages' section listing [[wikilinks]] to the Extracted pages above
...
def build_source_page_messages(
    ...
    """Pass 3 — draft the Metadata/<slug>.md summary page."""
    takeaways_str = "\n".join(f"- {t}" for t in key_takeaways)
    extracted_links = (
        "\n".join(f"- [[Extracted/{s}]]" for s in extracted_slugs)
        if extracted_slugs
        else "  (none)"
    )
