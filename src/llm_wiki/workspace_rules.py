...
This module installs llm-wiki managed rule files into an arbitrary workspace
...
MANAGED_START = "<!-- llm-wiki:start -->"
MANAGED_END = "<!-- llm-wiki:end -->"

ANTIGRAVITY_START = "# llm-wiki:start"
ANTIGRAVITY_END = "# llm-wiki:end"
...
def prepare_workspace(
    *,
    ...
    Files under `.agents/llm_wiki/` are owned by llm-wiki and are overwritten on
    ...
    if install_rules and agent != "none":
        _install_rule_templates(wiki_root, workspace, agent, result)
...
def _render_template(rel_path: str, values: dict[str, str]) -> str:
    text = (_template_root() / rel_path).read_text(encoding="utf-8")
...
def _install_rule_templates(
    ...
    owned_templates = {
        "owned/shared/rules.md": ".agents/llm_wiki/shared/rules.md",
        "owned/shared/sync.md": ".agents/llm_wiki/shared/sync.md",
        "owned/shared/check_rule_sync.py": ".agents/llm_wiki/shared/check_rule_sync.py",
        "owned/workflows/research_pipeline.md": ".agents/llm_wiki/workflows/research_pipeline.md",
        "owned/workflows/daily_log_push.md": ".agents/llm_wiki/workflows/daily_log_push.md",
        f"owned/runtime/{agent}.md": f".agents/llm_wiki/runtime/{agent}.md",
    }
    for src, dest in owned_templates.items():
        _write_file(workspace / dest, _render_template(src, values), result)

    target_path, block_template = _top_level_target(agent)
    block = _render_template(block_template, values)
